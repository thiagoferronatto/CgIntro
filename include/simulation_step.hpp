...
template <typename T> void simulateStep(const CollisionProps<T> &props) {
  using namespace glm;

  constexpr float restitution{0.9};
  ...
  auto j{
      ...
      (dot(n, n * (invma + invmb)) + dot(cross((invma * cross(ra, n)), ra) +
                                         ...
                                         n))};
  body1._velocity += invma * j * n;
  body2._velocity -= invmb * j * n;
  body1._angularVelocity += invma * cross(ra, j * n);
  body2._angularVelocity += invmb * cross(rb, j * n);
